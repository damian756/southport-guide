...
export default async function ReviewsPage() {
  ...
  const points = chartSnapshots
    .map((s, i) => `${toX(i)},${toY(s.rating)}`)
    .join(" ");
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        {business.rating != null ? (
          <>
            ...
            {!business.placeId && (
              <p className="text-xs text-gray-400 mt-3">
                ...
                <a href="mailto:hello@southportguide.co.uk" className="text-[#C9A84C] font-medium hover:underline">
                  Contact us to connect it.
                </a>
              </p>
            )}
          </>
          ...
          <p className="text-sm text-gray-500">
            ...
            <a href="mailto:hello@southportguide.co.uk" className="text-[#C9A84C] font-semibold hover:underline">
              Contact us
            </a>{" "}
            to link your Google listing and start tracking your rating here.
          </p>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        {chartSnapshots.length >= 2 ? (
              ...
              {chartSnapshots.map((s, i) => {
                ...
                return (
                  <text
                    ...
                    fill="#9ca3af"
                  >
                    {new Date(s.snapshotAt).toLocaleDateString("en-GB", {
                      day: "numeric",
                      month: "short",
                    })}
                  </text>
          ...
          <p className="text-sm text-gray-500">
            We&apos;ll track your rating over time. Check back after the next
            review check.
          </p>
        )}
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {snapshots.map((snap, i) => {
                ...
                const reviewDelta = prev ? snap.reviewCount - prev.reviewCount : null;
                const ratingDelta = prev
                  ? parseFloat((snap.rating - prev.rating).toFixed(2))
                  : null;
                ...
                return (
                  ...
                  >
                    <td className="py-4 px-6 text-gray-700">
                      {new Date(snap.snapshotAt).toLocaleDateString("en-GB", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })}
                    </td>
                    <td className="py-4 px-6 text-right font-medium text-[#1B2E4B]">
                      {snap.rating.toFixed(1)} ★
                    </td>
                    <td className="py-4 px-6 text-right">
                      {snap.reviewCount.toLocaleString()}
                    </td>
                    <td className="py-4 px-6 text-right">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex items-start gap-3">
        ...
        <p className="text-sm text-gray-600">
          You&apos;ll receive email alerts when a new review is added or your
          rating changes. Manage notifications in{" "}
          <Link
