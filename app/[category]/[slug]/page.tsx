...
function buildMetaDescription(name: string, catName: string, area: string, description: string | null, shortDescription: string | null, rating: number | null, reviewCount: number | null): string {
  if (description) {
    // First 155 chars of AI description
...
export default async function BusinessPage({ params }: Props) {
  ...
  try {
    ...
    if (categoryRecord) {
      ...
      // Related listings – same category, weighted score, exclude self
      if (business) {
        ...
        <div className="container mx-auto px-4 py-8 max-w-5xl">
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="p-6">
                  ...
                  {business.rating && (
                    <div className="flex items-center gap-2 mb-4">
                      <span className="flex items-center gap-1 bg-amber-50 text-amber-700 font-semibold px-3 py-1.5 rounded-full border border-amber-200">
                        <Star className="w-4 h-4 fill-amber-400 text-amber-400" />
                        {business.rating.toFixed(1)}
                        {business.reviewCount && (
                          <span className="text-amber-600 font-normal">({business.reviewCount.toLocaleString()} reviews)</span>
                        )}
                      </span>
              ...
              {/* Google Maps embed */}
              {(business.placeId || (business.lat && business.lng)) && mapsKey && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                  <iframe
                    ...
                    src={
                      business.placeId
                        ? `https://www.google.com/maps/embed/v1/place?key=${mapsKey}&q=place_id:${business.placeId}`
                        : `https://www.google.com/maps/embed/v1/place?key=${mapsKey}&q=${business.lat},${business.lng}`
                    }
                    allowFullScreen
              ...
              {related.length > 0 && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    {related.map((r) => (
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500 truncate mt-0.5">{r.address.replace(/,?\s*(United Kingdom|UK)$/i, "").split(",").slice(-2).join(",").trim()}</p>
                        </div>
                  ...
                  <Link href={`/${category}`} className="block text-center text-blue-600 text-sm mt-4 hover:underline">
                    View all {cat.name} →
                  </Link>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5 space-y-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-800 text-sm">{area}{area !== "Southport" ? ", Southport" : ""}</p>
                  </div>
                </div>

                {business.priceRange && (
                  <div className="flex items-start gap-3">
                    <span className="w-4 h-4 text-blue-500 mt-0.5 flex-shrink-0 text-sm font-bold">£</span>
                    <div>
                      <p className="text-xs text-gray-500 mb-0.5">Price range</p>
                      <p className="text-gray-800 text-sm">{business.priceRange}</p>
                    </div>
                  </div>
                )}
              ...
              {!business.claimed && (
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-5">
                  ...
                  <p className="text-blue-700 text-sm mb-4">Claim your free listing to update details, add photos and attract more customers.</p>
                  <Link
                    href="/claim-listing"
